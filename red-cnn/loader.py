...
# https://discuss.pytorch.org/t/how-to-get-npy-dataloader/52379/2
class ct_patch_dataset(Dataset):
    def __init__(self, np_file_paths_x, np_file_paths_y):
        self.files_x = np_file_paths_x
        self.files_y = np_file_paths_y

    def __getitem__(self, index):
        x = np.load(self.files_x[0])
        x = torch.from_numpy(x).float()
        y = np.load(self.files_y[1])
...
# needs pre-defined map so


## This function reads big npy files from data_path and save it into save_path
def prep(data_path, save_path):
    os.makedirs(save_path)
    files = sorted(glob(os.path.join(data_path, '*.npy')))
    for i, f in enumerate(files):
        print('...Preparing file no. '+str(i)+'...')
        big_npy_file = np.load(f)

        for j in range(big_npy_file.shape[0]):
            new_filename = os.path.splitext(os.path.basename(f))[0] + '_' + str(j)
            if j == 1:
                print(big_npy_file[j,:,:,:].shape)
            np.save(os.path.join(save_path, new_filename), big_npy_file[j,:,:,:])
        del f
...
def get_loader(mode='train', load_mode=0,
    ...
    LDCT_filelist = sorted(glob(os.path.join(saved_path, 'train_LDCT_*.npy')))
    NDCT_filelist = sorted(glob(os.path.join(saved_path, 'train_NDCT_*.npy')))
    dataset_ = ct_patch_dataset(LDCT_filelist, NDCT_filelist)
