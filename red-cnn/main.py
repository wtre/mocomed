...
from loader import get_loader, prep
...
def main(args):
    ...
    if args.mode == 'train':
        if not (os.path.exists(args.saved_path) and os.listdir(args.saved_path)):
            prep(args.data_path, args.saved_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='../dataset/SACNN/patch_subset')
    ############ new folder for prepped image patches ############
    parser.add_argument('--saved_path', type=str, default='./npy_imgpatch/')
    ##############################################################
